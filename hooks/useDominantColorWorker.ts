...
export function useDominantColorWorker(id: string | undefined, imageUrl: string | undefined) {
    ...
    worker.onmessage = (event: MessageEvent) => {
      ...
      if (success) {
        ...
        if (returnedId && Array.isArray(color) && color.length === 3) {
          localStorage.setItem(`dominant-color-${returnedId}`, JSON.stringify(color));
        }
      } else {
        setColor(null);
