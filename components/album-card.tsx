...
export function AlbumCard({
	...
	return (
		<div
			ref={ref}
			className={cn(
				"relative p-4 rounded-xl overflow-hidden flex flex-col group",
				className,
			)}
			style={{ background: bg }}
		>
			...
			<div className="relative w-full aspect-square overflow-hidden rounded-lg shadow">
				<img
					src={imgUrl || "/placeholder.svg?height=200&width=200"}
					alt={album.name}
							...
							{album.artists.map(({ id, name }, i) => (
								<React.Fragment key={id}>
									<UniversalContextMenu type="artist" id={id}>
										<Link
											href={`/artists/${id}`}
											className="z-20 text-xs hover:underline"
										>
											{name}
										</Link>
									</UniversalContextMenu>
			...
			<div
				...
				style={{
					background:
						"linear-gradient(to top, rgba(24,24,24,0.5) 60%, transparent 100%)",
				}}
			/>
		</div>
	);
