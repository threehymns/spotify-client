...
export default function Player() {
	...
	return (
		<div className="fixed bottom-0 left-0 right-0 z-50 flex items-center justify-between px-6 h-20 bg-zinc-950/90 backdrop-saturate-200 backdrop-blur-md border-t border-zinc-800">
			{/* Track info */}
			<div className="flex items-center w-1/3">
						...
						<MotionAlbumArt
							...
							alt={
								currentTrack?.album?.name
									? currentTrack.album.name + " Album cover"
									: "No album"
							}
							fill
				...
				<motion.div className="min-w-0" layoutId="now-playing-track-info">
					<div
						className={`font-semibold truncate text-base ${!currentTrack ? "text-zinc-400" : "text-white drop-shadow"} transition-colors duration-150`}
					>
						{currentTrack ? (
							(() => {
			...
			<div className="flex flex-col items-center w-1/3">
				...
				>
					<div className="text-xs text-zinc-400 w-10 text-right font-mono tracking-tight">
						{formatTime(currentTrack ? currentPosition : 0)}
					</div>
					...
					<div className="text-xs text-zinc-400 w-10 font-mono tracking-tight">
