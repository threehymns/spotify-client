...
export default function PlaylistDetailPage({
	...
	const [playlist, setPlaylist] = useState<z.infer<
		typeof SpotifyPlaylist
	> | null>(null);
	...
	const handleFetchError = useCallback(
		(error: unknown, type: FetchState["type"]) => {
			...
			setFetchState((prev) => ({
				...
				loading: false,
				error:
					error instanceof Error ? error : new Error(`Failed to load ${type}`),
				type,
			}));
		},
		[],
		...
		const loadFollowStatus = async () => {
			if (id && !isOwner) {
				try {
					...
					setFetchState((prev) =>
						prev.type === "follow-status" ? { ...prev, loading: false } : prev,
					);
				}
			}
		};
