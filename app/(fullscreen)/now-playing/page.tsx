...
import { Play, Pause, SkipBack, SkipForward, Volume2, VolumeX, Minimize } from "lucide-react";
...
export default function NowPlayingPage() {
  ...
  return (
    <div className="relative flex flex-col items-center justify-center min-h-screen p-4 overflow-hidden">
      ...
      >
        {currentTrack ? (
          currentTrack.artists && currentTrack.artists[0]?.images?.[0]?.url ? (
            <img
      ...
      >
        <MotionAlbumArt
          src={currentTrack?.album?.images[0]?.url || "/placeholder.svg?height=240&width=240"}
      ...
      >
        <div className={`text-2xl font-bold truncate ${!currentTrack ? 'text-zinc-400' : 'text-white'}`}>
          {currentTrack ? (() => {
      ...
      <motion.div
        ...
        animate={{ opacity: showControls ? 1 : 0, y: showControls ? 0 : 10, scale: showControls ? 1 : 0.75 }}
        transition={{
      ...
      >
        ...
        >
          {isPlaying ? <Pause className="!size-10" fill="#000" strokeWidth={1} /> : <Play className="!size-10" fill="black" />}
        </Button>
      ...
      <motion.div
        ...
        animate={{ opacity: showControls ? 1 : 0, y: showControls ? 0 : 10, scale: showControls ? 1 : 0.98 }}
        transition={{ delay: hasAnimatedOnce.current && showControls ? 0.23 : 0 }}
        className="flex items-center gap-2 w-full max-w-xl mx-auto z-30"
      >
        <div className="text-xs text-zinc-400 w-12 text-right">{formatTime(currentTrack ? position : 0)}</div>
        <Slider
        ...
        />
        <div className="text-xs text-zinc-400 w-12">{formatTime(currentTrack ? duration : 0)}</div>
      </motion.div>
      ...
      <div className="absolute bottom-8 right-8 flex items-center gap-4 z-40">
        <motion.div
          ...
          animate={{ opacity: showControls ? 1 : 0, y: showControls ? 0 : 10, scale: showControls ? 1 : 0.98 }}
          transition={{ delay: hasAnimatedOnce.current && showControls ? 0.28 : 0 }}
          className="flex items-center gap-2"
        >
          <Button size="icon" variant="ghost" onClick={() => setMute(!isMuted)} disabled={!currentTrack}>
